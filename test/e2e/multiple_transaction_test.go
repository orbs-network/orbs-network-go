...
func sendAmount(t *testing.T, h *harness, amount uint64, signerKeyPair *keys.Ed25519KeyPair, targetAddress primitives.Ripmd160Sha256) {
...
func TestNetworkCommitsMultipleTransactions(t *testing.T) {
	...
	for _, amount := range amounts {
		...
		sendAmount(t, h, amount, signerKeyPair, targetAddress)
	}
...
func TestMultipleTransactionsInBatches(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping E2E tests in short mode")
	}
	...
	signerKeyPair := keys.Ed25519KeyPairForTests(8)
	targetAddress := builders.AddressForEd25519SignerForTests(9)

	sendAmount(t, h, 0, signerKeyPair, targetAddress)

	for _, bracket := range amounts {
		for _, amount := range bracket {
			go sendAmount(t, h, amount, signerKeyPair, targetAddress)
		}
	}

	// check balance
	ok := test.Eventually(test.EVENTUALLY_DOCKER_E2E_TIMEOUT, func() bool {
		signerKeyPair := keys.Ed25519KeyPairForTests(9)
		targetAddress := builders.AddressForEd25519SignerForTests(9)
		getBalance := builders.GetBalanceTransaction().WithEd25519Signer(signerKeyPair).WithTargetAddress(targetAddress).Builder().Transaction
		response, err := h.callMethod(t, getBalance)
		if err == nil && response.CallMethodResult() == protocol.EXECUTION_RESULT_RESERVED { // TODO: this is a bug, change to EXECUTION_RESULT_SUCCESS
