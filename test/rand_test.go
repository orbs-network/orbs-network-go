...
func TestWithRandLogsCorrectSeedAndTestName(t *testing.T) {
	...
	randUint1 := NewRand(&nlMock).Uint64()
	expectedrandUint1 := rand.New(rand.NewSource(loggedSeed)).Uint64()
	require.Equal(t, expectedrandUint1, randUint1, "expected NewRand() to log the correct random seed")
}
func TestWithExplicitRand(t *testing.T) {
	...
	randUint1 := NewRand(&nlMock).Uint64()
	randUint2 := NewRand(&nlMock).Uint64()
