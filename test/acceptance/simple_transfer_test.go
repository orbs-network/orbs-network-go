...
func TestLeaderCommitsTransactionsAndSkipsInvalidOnes(t *testing.T) {
	harness.Network(t).Start(func(network harness.InProcessNetwork) {
		...
		tx1 := <-network.SendTransfer(0, 17, 5, 6)
		<-network.SendInvalidTransfer(0, 5, 6)
		tx2 := <-network.SendTransfer(0, 22, 5, 6)
		...
		require.EqualValues(t, benchmarktoken.TOTAL_SUPPLY-39, <-network.CallGetBalance(0, 5), "getBalance result on leader")
		require.EqualValues(t, 39, <-network.CallGetBalance(0, 6), "getBalance result on leader")
		...
		require.EqualValues(t, benchmarktoken.TOTAL_SUPPLY-39, <-network.CallGetBalance(1, 5), "getBalance result on non leader")
		require.EqualValues(t, 39, <-network.CallGetBalance(1, 6), "getBalance result on non leader")
...
func TestNonLeaderPropagatesTransactionsToLeader(t *testing.T) {
	harness.Network(t).Start(func(network harness.InProcessNetwork) {
		...
		txHash := network.SendTransferInBackground(1, 17, 5, 6)
		...
		require.EqualValues(t, 0, <-network.CallGetBalance(0, 6), "initial getBalance result on leader")
		require.EqualValues(t, 0, <-network.CallGetBalance(1, 6), "initial getBalance result on non leader")

		pausedTxForwards.Release()
		network.WaitForTransactionInState(0, txHash)
		require.EqualValues(t, 17, <-network.CallGetBalance(0, 6), "eventual getBalance result on leader")
		network.WaitForTransactionInState(1, txHash)
		require.EqualValues(t, 17, <-network.CallGetBalance(1, 6), "eventual getBalance result on non leader")
