...
func TestBenchmarkConsensusLeaderGetsVotesBeforeNextBlock(t *testing.T) {
	harness.Network(t).Start(func(network harness.InProcessNetwork) {
		...
		network.SendTransferInBackground(0, 0, 5, 6) // send a transaction so that network advances to block 1. the tamper prevents COMMITTED messages from reaching leader, so it doesn't move to block 2
		...
		txHash := network.SendTransferInBackground(0, 17, 5, 6) // this should be included in block 2 which will not be closed until leader knows network is at block 2
