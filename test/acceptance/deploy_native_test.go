...
func TestNonLeaderDeploysNativeContract(t *testing.T) {
	...
	harness.Network(t).Start(func(network harness.AcceptanceTestNetwork) {
		...
		require.EqualValues(t, counterStart, <-network.CallCounterGet(1, counterStart), "get counter after deploy")

		t.Log("transacting with contract")

		<-network.SendCounterAdd(1, counterStart, 17)
		require.EqualValues(t, counterStart+17, <-network.CallCounterGet(1, counterStart), "get counter after transaction")
