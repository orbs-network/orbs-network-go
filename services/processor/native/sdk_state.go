...
func (s *stateSdk) ReadBytesByKey(executionContextId sdk.Context, key string) ([]byte, error) {
	address := keyToAddress(key)
	return s.ReadBytesByAddress(executionContextId, address)
}

func (s *stateSdk) ReadStringByAddress(executionContextId sdk.Context, address sdk.Ripmd160Sha256) (string, error) {
	bytes, err := s.ReadBytesByAddress(executionContextId, address)
	return string(bytes), err
}

func (s *stateSdk) ReadStringByKey(executionContextId sdk.Context, key string) (string, error) {
	address := keyToAddress(key)
	return s.ReadStringByAddress(executionContextId, address)
}

func (s *stateSdk) ReadUint64ByAddress(executionContextId sdk.Context, address sdk.Ripmd160Sha256) (uint64, error) {
	bytes, err := s.ReadBytesByAddress(executionContextId, address)
...
func (s *stateSdk) ReadUint64ByKey(executionContextId sdk.Context, key string) (uint64, error) {
	address := keyToAddress(key)
	return s.ReadUint64ByAddress(executionContextId, address)
}

func (s *stateSdk) ReadUint32ByAddress(executionContextId sdk.Context, address sdk.Ripmd160Sha256) (uint32, error) {
	bytes, err := s.ReadBytesByAddress(executionContextId, address)
	if err != nil || len(bytes) == 0 {
		return 0, nil
	}
...
func (s *stateSdk) ReadUint32ByKey(executionContextId sdk.Context, key string) (uint32, error) {
	address := keyToAddress(key)
	return s.ReadUint32ByAddress(executionContextId, address)
}

func (s *stateSdk) WriteBytesByKey(executionContextId sdk.Context, key string, value []byte) error {
	address := keyToAddress(key)
	return s.WriteBytesByAddress(executionContextId, address, value)
}

func (s *stateSdk) WriteStringByAddress(executionContextId sdk.Context, address sdk.Ripmd160Sha256, value string) error {
	bytes := []byte(value)
	return s.WriteBytesByAddress(executionContextId, address, bytes)
}

func (s *stateSdk) WriteStringByKey(executionContextId sdk.Context, key string, value string) error {
	address := keyToAddress(key)
	return s.WriteStringByAddress(executionContextId, address, value)
}

func (s *stateSdk) WriteUint64ByAddress(executionContextId sdk.Context, address sdk.Ripmd160Sha256, value uint64) error {
	bytes := make([]byte, 8)
	membuffers.WriteUint64(bytes, value) // TODO: maybe we need WriteUint64Polyfill if we cannot guarantee alignment
	return s.WriteBytesByAddress(executionContextId, address, bytes)
}

func (s *stateSdk) WriteUint64ByKey(executionContextId sdk.Context, key string, value uint64) error {
	address := keyToAddress(key)
	return s.WriteUint64ByAddress(executionContextId, address, value)
}

func (s *stateSdk) WriteUint32ByAddress(executionContextId sdk.Context, address sdk.Ripmd160Sha256, value uint32) error {
	bytes := make([]byte, 4)
	membuffers.WriteUint32(bytes, value) // TODO: maybe we need WriteUint32Polyfill if we cannot guarantee alignment
	return s.WriteBytesByAddress(executionContextId, address, bytes)
}

func (s *stateSdk) WriteUint32ByKey(executionContextId sdk.Context, key string, value uint32) error {
	address := keyToAddress(key)
	return s.WriteUint32ByAddress(executionContextId, address, value)
}

func (s *stateSdk) ClearByAddress(executionContextId sdk.Context, address sdk.Ripmd160Sha256) error {
	return s.WriteBytesByAddress(executionContextId, address, []byte{})
}

func (s *stateSdk) ClearByKey(executionContextId sdk.Context, key string) error {
	address := keyToAddress(key)
	return s.ClearByAddress(executionContextId, address)
}

func keyToAddress(key string) sdk.Ripmd160Sha256 {
	return sdk.Ripmd160Sha256(hash.CalcRipmd160Sha256([]byte(key)))
}
