...
func TestForwardsANewValidTransactionUsingGossip(t *testing.T) {
	with.Concurrency(t, func(ctx context.Context, parent *with.ConcurrencyHarness) {
		...
		sig, _ := signer.NewLocalSigner(thisNodeKeyPair.PrivateKey()).Sign(ctx, hash)
...
func TestDoesNotAddTransactionWithFutureProtocolVersion(t *testing.T) {
	with.Concurrency(t, func(ctx context.Context, parent *with.ConcurrencyHarness) {
		...
		tx := builders.TransferTransaction().WithProtocolVersion(config.MAXIMAL_CONSENSUS_BLOCK_PROTOCOL_VERSION +1).Build()
		h.ignoringForwardMessages()
		h.ignoringTransactionResults()
...
func TestDoesAddTransactionWithOldProtocolVersion(t *testing.T) {
	with.Concurrency(t, func(ctx context.Context, parent *with.ConcurrencyHarness) {
		...
		tx := builders.TransferTransaction().WithProtocolVersion(config.MINIMAL_CONSENSUS_BLOCK_PROTOCOL_VERSION -1).Build()
		h.ignoringForwardMessages()
		h.ignoringTransactionResults()
