...
func TestValidateTransaction_Add_InvalidTransactions(t *testing.T) {
	...
	tests := []struct {
		...
		{"protocol version", aTransactionAtNodeTimestamp(lastCommittedBlockTime).WithProtocolVersion(config.MAXIMAL_CONSENSUS_BLOCK_PROTOCOL_VERSION + 1), protocol.TRANSACTION_STATUS_REJECTED_UNSUPPORTED_VERSION},
...
func TestValidateTransaction_Ordering_InvalidTransactions(t *testing.T) {
	...
	tests := []struct {
		...
		{"protocol version", aTransactionAtNodeTimestamp(proposedBlockTime).WithProtocolVersion(config.MAXIMAL_CLIENT_PROTOCOL_VERSION + 1), protocol.TRANSACTION_STATUS_REJECTED_UNSUPPORTED_VERSION},
