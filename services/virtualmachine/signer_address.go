...
func (s *service) getSignerAddress(signer *protocol.Signer) (primitives.Ripemd160Sha256, error) {
...
func addressEd25519Signer(signer *protocol.Signer) (primitives.Ripemd160Sha256, error) {
	...
	return hash.CalcRipemd160Sha256(signerPublicKey), nil
}

// TODO(v1): add argument (spec feature)
func addressContractCall(contractName primitives.ContractName) (primitives.Ripemd160Sha256, error) {
	if len(contractName) == 0 {
		return nil, errors.New("contract name is missing for addressing")
	}
	return hash.CalcRipemd160Sha256([]byte(contractName)), nil
}
