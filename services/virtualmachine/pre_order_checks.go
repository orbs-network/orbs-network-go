...
func (s *service) verifyCommitteeStatus(blockTime primitives.TimestampNano, referenceTime primitives.TimestampSeconds) bool {
	return time.Duration(referenceTime)*time.Second+s.cfg.CommitteeGracePeriod() >= time.Duration(blockTime)
}
