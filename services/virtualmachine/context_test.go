...
func TestContext_Load(t *testing.T) {
	...
	contextId1, _ := cp.allocateExecutionContext(0, 1, 0x222, []byte{0x01}, 8, 7, protocol.ACCESS_SCOPE_READ_ONLY, nil)
	defer cp.destroyExecutionContext(contextId1)

	contextId2, _ := cp.allocateExecutionContext(4, 5, 0x666, []byte{0x02}, 9, 8, protocol.ACCESS_SCOPE_READ_ONLY, nil)
	defer cp.destroyExecutionContext(contextId1)
	...
	require.EqualValues(t, []byte{0x01}, c1.currentBlockProposerAddress, "loaded context with contextId1 should be currentBlockProposerAddress 0x01")
	require.EqualValues(t, 8, c1.currentBlockReferenceTime, "loaded context with contextId1 should be currentBlockReferenceTime 8")
	require.EqualValues(t, 7, c1.lastBlockReferenceTime, "loaded context with contextId1 should be lastBlockReferenceTime 8")
	...
	require.EqualValues(t, []byte{0x02}, c2.currentBlockProposerAddress, "loaded context with contextId2 should be currentBlockProposerAddress 0x02")
	require.EqualValues(t, 9, c2.currentBlockReferenceTime, "loaded context with contextId1 should be currentBlockReferenceTime 9")
	require.EqualValues(t, 8, c2.lastBlockReferenceTime, "loaded context with contextId1 should be lastBlockReferenceTime 9")
}

func TestContext_ServiceStack(t *testing.T) {
	cp := newExecutionContextProvider()
	executionContextId, c := cp.allocateExecutionContext(0, 1, 0x222, []byte{0x01}, 8, 7, protocol.ACCESS_SCOPE_READ_ONLY, nil)
...
func TestContext_EventList(t *testing.T) {
	cp := newExecutionContextProvider()
	executionContextId, c := cp.allocateExecutionContext(0, 1, 0x222, []byte{0x01}, 9, 8, protocol.ACCESS_SCOPE_READ_ONLY, nil)
	defer cp.destroyExecutionContext(executionContextId)
