...
func TestPublicApiGetTxProof_PrepareResponseNilProof(t *testing.T) {
	...
	test.RequireCmpEqual(t, (*protocol.ReceiptProofBuilder)(nil).Build(), response.ClientResponse.Proof(), "Transaction receipt is not equal")
	require.Equal(t, 0, len(response.ClientResponse.Proof().Raw()), "Transaction receipt is not equal") // different way
...
func TestPublicApiGetTxProof_EmptyResponse(t *testing.T) {
	...
	test.RequireCmpEqual(t, (*protocol.ReceiptProofBuilder)(nil).Build(), response.ClientResponse.Proof(), "Transaction proof is not equal")
	require.Equal(t, 0, len(response.ClientResponse.Proof().Raw()), "Transaction proof is not equal") // different way
