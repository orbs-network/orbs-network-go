...
The last node in the path is:
1. inclusion: the leaf-node's `value-hash`
1. exclusion: the first node whose `prefix` contributes to a _path-prefix_ that does not coincide with `requested_key`.

This allows verifying the proof by calculating "bottom-up" the node hashes, arriving back at the Merkle Root.

Any proof is made of:
...
1. _path-prefix_ - padded up to 32 bytes. if this is an inclusion proof the _path-prefix_ may be shorter than 32 bytes, however, the trailing bits may be ignored
