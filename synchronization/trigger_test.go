...
func TestPeriodicalTriggerStartsOk(t *testing.T) {
	...
	p := synchronization.NewPeriodicalTrigger(context.Background(), tickTime, func() { atomic.AddUint32(&x, 1) }, nil)
...
func TestPeriodicalTrigger_Stop(t *testing.T) {
	x := 0
	p := synchronization.NewPeriodicalTrigger(context.Background(), time.Millisecond*2, func() { x++ }, nil)
	p.Stop()
...
func TestPeriodicalTrigger_StopAfterTrigger(t *testing.T) {
	x := 0
	p := synchronization.NewPeriodicalTrigger(context.Background(), time.Millisecond, func() { x++ }, nil)
...
func TestPeriodicalTriggerStopOnContextCancel(t *testing.T) {
	...
	x := 0
	synchronization.NewPeriodicalTrigger(ctx, time.Millisecond*2, func() { x++ }, nil)
	cancel()
	time.Sleep(3 * time.Millisecond)
	require.Equal(t, 0, x, "expected no ticks")
}
...
func TestPeriodicalTriggerStopWorksWhenContextIsCancelled(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	x := 0
	p := synchronization.NewPeriodicalTrigger(ctx, time.Millisecond*2, func() { x++ }, nil)
	cancel()
	time.Sleep(3 * time.Millisecond)
	require.Equal(t, 0, x, "expected no ticks")
	p.Stop()
...
func TestPeriodicalTriggerStopOnContextCancelWithStopAction(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	x := 0
	synchronization.NewPeriodicalTrigger(ctx, time.Millisecond*2, func() { x++ }, func() { x = 20 })
	cancel()
...
func TestPeriodicalTriggerRunsOnStopAction(t *testing.T) {
	x := 0
	p := synchronization.NewPeriodicalTrigger(context.Background(), time.Second, func() { x++ }, func() { x = 20 })
	p.Stop()
	require.Equal(t, 20, x, "expected x to have the stop value")
}
