...
func newFromJSON(source string) (*MapBasedConfig, error) {
	return newFromJSONWithParent(emptyConfig(), source)
}

func newFromJSONWithParent(parent *MapBasedConfig, source string) (*MapBasedConfig, error) {
		...
		for _, item := range nodeList {
			...
			if nodeAddress, err := hex.DecodeString(hexAddress); err != nil {
				...
				if i, err := parseUint32(kv["port"].(float64)); err != nil {
					...
					gossipPort := int(i)
					peers[nodeAddress.KeyForMap()] = NewHardCodedGossipPeer(gossipPort, kv["ip"].(string), hexAddress)
				}
			}
		}
	}
